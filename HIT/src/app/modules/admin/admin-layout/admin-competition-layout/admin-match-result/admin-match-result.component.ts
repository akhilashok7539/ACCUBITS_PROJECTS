...
@Component({
  selector: 'app-admin-match-result',
  templateUrl: './admin-match-result.component.html',
  styleUrls: ['./admin-match-result.component.scss']
})
export class AdminMatchResultComponent implements OnInit {
  ...
  predictionsResult(round?){
    ...
    this.service.httpGet('/v1/admin/competition/predictions/results?referenceId=' + this.competition_detail.guid + '&' + params).subscribe((response) => {
      this.error ='';
  ...
  showPlayersList() {
    this.showLeaderBoardPlayers = !this.showLeaderBoardPlayers;
  }
}
