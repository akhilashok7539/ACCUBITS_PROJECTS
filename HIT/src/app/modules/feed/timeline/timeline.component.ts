...
export class TimelineComponent implements OnInit {
  ...
  viewGame(item) {
    if (item.competition.isPlayer === 0) {
      localStorage.setItem('go_to_feed', 'true')
      this.router.navigateByUrl('/competition/details/' + item.competition.guid)
    } else {
      localStorage.setItem('go_to_feed', 'true')
      this.router.navigateByUrl('/competition/betting/' + item.competition.guid)
    }
    localStorage.setItem('competition_view_active_index' , '1');

  }
  viewGameChamp(item) {
    if (item.competition.isRoundPlayer === 0 && item.competition.isFullPlayer === 0) {
      localStorage.setItem('go_to_feed', 'true')
      this.router.navigateByUrl('/competition/details/' + item.competition.guid)
    } else {
      localStorage.setItem('go_to_feed', 'true')
  ...
  viewUserProfile(data)
  {
    ...
    this.router.navigateByUrl('/feed/profile?user='+data.guid);
